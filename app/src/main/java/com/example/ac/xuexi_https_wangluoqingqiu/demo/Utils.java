package com.example.ac.xuexi_https_wangluoqingqiu.demo;
...
public class Utils {
    ...
    public void getUtilsManag(Context context, InterApi<DomeBean> domeBeanInterApi) {

        ThreadPoolManager.getInstance().execute(new Runnable() {
            @Override
            public void run() {
                DomeBean domeBean = null;
                domeBean = new DomeBean();
                domeBean.setA(isNetConnect(context));//是否有网络连�?
                domeBean.setB(isWifiConnect(context));//是否为wifi连接状态?
                domeBean.setC(haveSDCard());// 是否安装了sdcard?
                domeBean.setD(getSystemAvailableSize());//获取系统内部可用空间大小 long
                domeBean.setE(getSDCardAvailableSize());//获取sd卡可用空间大�? long
                domeBean.setF(getBattery(context));//获取电池电量,0~1 float
                domeBean.setG(getMobileName());// 获取手机名称 String
                domeBeanInterApi.complete(domeBean);
            }
        });
     ...
     *
     * @param pxValue
     * @param context （DisplayMetrics类中属�?�density�?
     * @return
     */
     ...
     *
     * @param pxValue
     * @param context （DisplayMetrics类中属�?�scaledDensity�?
     * @return
     */

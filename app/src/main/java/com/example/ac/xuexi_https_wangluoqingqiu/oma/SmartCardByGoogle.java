...
import com.example.ac.xuexi_https_wangluoqingqiu.utils.Utils;
import com.example.ac.xuexi_https_wangluoqingqiu.utils.omautils.Hex;
...
public final class SmartCardByGoogle extends BaseSmartCard implements SEService.OnConnectedListener {
    ...
    @Override
    public CardResult openChannel(String command) {//开通道
        try {
            bindService();
    ...
    private Reader getCurrentAvailableReader() {
        ...
        Reader[] readers = mSEService.getReaders();//SIM reader ;ese 手机芯片 reader
        // 判断是否有可用的通道
        if (readers.length < 1) {
